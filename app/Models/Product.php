...
class Product extends Model
{
    ...
    {
        return $this->belongsToMany(ProductGroup::class, 'product_x_product_group', 'product_uuid', 'product_group_uuid')
            ->withPivot('order')
            ->withTimestamps();
    }
    ...
    {
        return $this->belongsToMany(ProductAttribute::class, 'product_x_product_attribute', 'product_uuid', 'product_attribute_uuid')
            ->withTimestamps();
    }

    public function productOptionGroups(): BelongsToMany
    {
        return $this->belongsToMany(ProductOptionGroup::class, 'product_x_product_option_group', 'product_uuid', 'product_option_group_uuid')
            ->withPivot('order')
            ->withTimestamps();
    }
        ...
        foreach ($defaultPrices as $defaultPrice) {
            foreach ($currencies as $currency) {
                ...
                $data = [
                    ...
                    'setup' => is_null($defaultPrice->setup) ? null : round($defaultPrice->setup * $rate, 4),
                    'base' => is_null($defaultPrice->base) ? null : round($defaultPrice->base * $rate, 4),
                    'idle' => is_null($defaultPrice->idle) ? null : round($defaultPrice->idle * $rate, 4),
                    'switch_down' => is_null($defaultPrice->switch_down) ? null : round($defaultPrice->switch_down * $rate, 4),
                    'switch_up' => is_null($defaultPrice->switch_up) ? null : round($defaultPrice->switch_up * $rate, 4),
                    'uninstall' => is_null($defaultPrice->uninstall) ? null : round($defaultPrice->uninstall * $rate, 4),
                ];
