@section('content')
    ...
    <div class="container px-0">
        ...
        <div class="card mb-2 shadow-sm border-0" style="min-height: 250px; position: relative; z-index: 1;">
            <div class="card-body text-center" id="app_status_card">
                <div class="mb-4">
                    <div id="status_spinner" class="spinner-border text-primary" style="width: 4rem; height: 4rem;" role="status"></div>
                </div>

                <h4 class="fw-bold text-dark" id="status_message">{{ __('Product.puqProxmox.Deploying your application') }}</h4>

                <p class="small text-muted mt-2" id="sub_message">{{ __('Product.puqProxmox.Please wait while your application is being prepared') }}</p>
    ...
    <script>
        $(document).ready(function () {
            ...
            function updateAppCard(data) {
                ...
                let statusMessage = "{{ __('Product.puqProxmox.Deploying your application') }}";
                let subMessage = "{{ __('Product.puqProxmox.Please wait while your application is being prepared') }}";
                let colorClass = 'text-secondary';
                let progressClass = 'bg-secondary';

                switch(status) {
                    case 'pending':
                        statusMessage = "{{ __('Product.puqProxmox.Application deployment is pending') }}";
                        colorClass = 'text-secondary';
                        progressClass = 'bg-secondary';
                        break;
                    case 'running':
                        statusMessage = "{{ __('Product.puqProxmox.Application deployment is running') }}";
                        colorClass = 'text-primary';
                        progressClass = 'bg-primary';
                        break;
                    case 'success':
                        statusMessage = "{{ __('Product.puqProxmox.Application deployed successfully') }}";
                        subMessage = "";
            ...
            function checkAppStatus() {
                ...
                PUQajax("{{ route('client.api.cloud.service.module.get', ['uuid' => $service_uuid, 'method' => 'getAppDeployStatus']) }}", {}, 50, null, 'GET')
                    .then(res => { updateAppCard(res.data); })
                    .catch(() => {});
            }
