...
@section('content')
    ...
    <div class="main-card mb-3 card">
        <div class="card-body">
            <table style="width: 100%;" id="cluster_groups"
                   class="table table-hover table-striped table-bordered">
                <thead>
                <tr>
                    <th>{{__('Product.puqProxmox.Name')}}</th>
                    <th>{{__('Product.puqProxmox.Fill Type')}}</th>
                    <th>{{__('Product.puqProxmox.Country')}}</th>
                    <th>{{__('Product.puqProxmox.Region')}}</th>
                    <th>{{__('Product.puqProxmox.Clusters')}}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            var tableId = '#cluster_groups';
            var ajaxUrl = '{{ route('admin.api.Product.puqProxmox.cluster_groups.get') }}';
            var columnsConfig = [
                {data: "name", name: "name"},
                ...
                {data: "region", name: "region"},
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return data.map(function (cluster) {
                            return `<a href="${cluster.url}" target="_blank" class="badge bg-primary me-1">${cluster.name}</a>`;
                        }).join(' ');
                    }
                },
                {
            ...
            <form id="createForm" class="col-md-10 mx-auto">
                ...
                <div class="mb-3">
                    <label class="form-label" for="fill_type">{{ __('Product.puqProxmox.Fill Type') }}</label>
            ...
            $('#modalSaveButton').on('click', function (event) {
                ...
                if ($('#createForm').length) {
                    ...
                    PUQajax('{{route('admin.api.Product.puqProxmox.cluster_group.post')}}', formData, 1000, $(this), 'POST', $form)
                        .then(function (response) {
