...
@section('buttons')
    <button type="button"
            ...
            id="addPublicNetwork">
        <i class="fa fa-plus"></i> <i class="fa fa-network-wired"></i> {{ __('Product.puqProxmox.Add Public Network') }}
    </button>
@endsection
...
@section('content')
    ...
    <div id="container">
        ...
        <div class="main-card mb-3 card">
            <div class="card-body">
                <table style="width: 100%;" id="public_networks" class="table table-hover table-striped table-bordered">
                    <thead>
                    <tr>
                        <th>{{ __('Product.puqProxmox.Name') }}</th>
                        <th>{{ __('Product.puqProxmox.MAC Pool') }}</th>
                        <th>{{ __('Product.puqProxmox.IP Pool') }}</th>
                        <th>{{ __('Product.puqProxmox.Bridge') }}/{{ __('Product.puqProxmox.Vlan Tag') }}</th>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            var tableId = '#public_networks';
            var ajaxUrl = '{{ route('admin.api.Product.puqProxmox.cluster.public_networks.get', $uuid) }}';
            var columnsConfig = [
                {
                    data: "name",
                    name: "name",
        ...
        `;
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if (!data) {
                            return '<span class="badge bg-info">DHCP</span>';
                        }

                        return `
            <div>
                <strong>${data}</strong><br>
                <small class="text-muted">${row.first_ip} – ${row.last_ip}</small>
            </div>
        `;
                    }
                },
            ...
            <div>
                <strong>${data}</strong><br>
                <small class="text-muted">VLAN ${row.vlan_tag}</small>
            </div>
        `;
                    }
                },
            ...
            $('#addPublicNetwork').on('click', function () {
                ...
                var formHtml = `
                    <form id="createForm" class="col-md-10 mx-auto">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name" placeholder="ex: public-network-01" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" min=0 max=4096 class="form-control" id="vlan_tag" name="vlan_tag" placeholder="0" required>
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="puq_pm_ip_pool_uuid" class="form-label">{{ __('Product.puqProxmox.IP Pool') }}</label>
                            <select name="puq_pm_ip_pool_uuid" id="puq_pm_ip_pool_uuid" class="form-select mb-2 form-control"></select>
                        </div>
                    </form>`;
                ...
                initializeSelect2(
                    $("#puq_pm_ip_pool_uuid"),
                    '{{ route('admin.api.Product.puqProxmox.ip_pools.select.get') }}',
                    {},
                    'GET',
                    1000,
                    {
            ...
            <form id="editForm" class="col-md-10 mx-auto">
                ...
                PUQajax(modelUrl, {}, 50, $(this), 'GET')
                    .then(function (response) {
                        ...
                        $('#vlan_tag').val(response.data.vlan_tag);
                        ...
                        initializeSelect2(
                            $("#puq_pm_ip_pool_uuid"),
                            '{{ route('admin.api.Product.puqProxmox.ip_pools.select.get') }}',
                            response.data.puq_pm_ip_pool_data,
                            'GET',
                            1000,
                            {
                                dropdownParent: $('#universalModal')
            ...
            $('#modalSaveButton').on('click', function (event) {
                ...
                if ($('#createForm').length) {
                    ...
                    PUQajax('{{ route('admin.api.Product.puqProxmox.public_network.post') }}', formData, 500, $(this), 'POST', $form)
                        .then(function (response) {
