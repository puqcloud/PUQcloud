                        ...
                        @if($log)
                            <div class="d-flex justify-content-between align-items-center small mt-2">
                                ...
                                <button class="btn btn-sm btn-outline-primary btn-icon-only view-script"
                                        ...
                                        data-url="{{ route('admin.api.Product.puqProxmox.puq_pm_app_instance.script_log.get',['uuid'=> $app_instance->uuid, 'log_uuid'=>$log['uuid']]) }}">
    ...
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 col-xl-4 col-xxl-4">
        <div class="card border-info shadow-sm">
            <div class="card-header bg-info text-white d-flex align-items-center">
                <i class="fas fa-cube me-2"></i> APP Info
            </div>
            <div class="card-body text-center">

                @if(!empty($app_instance_info['ssl_certificate']))
                    @php
                        $cert = $app_instance_info['ssl_certificate'];
                        $status = $cert['status'];
                        $now = \Carbon\Carbon::now();
                        $expires_at = isset($cert['expires_at']) ? \Carbon\Carbon::parse($cert['expires_at']) : null;
                        $expired = $expires_at && $expires_at->lt($now);
                    @endphp
                    ...
                    <div class="card shadow-sm mb-3 p-2">
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-2 fw-bold">
                            <div>
                                <i class="fas fa-certificate text-primary me-2"></i> SSL Certificate
                            </div>
                        </div>
                        ...
                        <div class="d-flex mb-1">
                            ...
                            @else
                                <span class="text-success">{{ ucfirst($status) }}</span>
                            @endif
                        </div>

                        @if(isset($cert['issued_at']))
                            <div class="d-flex mb-1">
                                <span class="fw-bold me-2">
                                    <i class="fas fa-calendar-check text-success me-1"></i> Issued At:
                                </span>
                                <span
                                    class="text-muted">{{ \Carbon\Carbon::parse($cert['issued_at'])->format('Y-m-d H:i') }}</span>
                            </div>
                        @endif
                        ...
                        @if(isset($cert['expires_at']))
                            <div class="d-flex">
                                <span class="fw-bold me-2">
                                    <i class="fas fa-calendar-times text-danger me-1"></i> Expires At:
                                </span>
                                <span class="{{ $expired ? 'text-danger fw-bold' : 'text-muted' }}">
                                    {{ \Carbon\Carbon::parse($cert['expires_at'])->format('Y-m-d H:i') }}
                                </span>
                            </div>
                        @endif
                ...
                @else
                    <p class="text-muted">No SSL certificate data.</p>
                @endif

                @if(empty($app_instance))
                    <div class="text-center p-5">
                        <i class="fa-solid fa-box-open text-danger" style="font-size: 4rem;"></i>
                        <h3 class="mt-3 text-danger fw-bold">
                            {{ __('Product.puqProxmox.APP instance not found') }}
                        </h3>
                        <p class="text-muted">
                            {{ __('Product.puqProxmox.The requested APP container could not be located') }}
                        </p>
                    </div>
                @else

                    @if(!empty($app_instance_info['endpoints']))
                        <button id="put_web_dns_records" type="button"
                                class="btn btn-outline-warning btn-sm d-flex align-items-center mb-2">
                            <i class="fa fa-sync me-1"></i> {{__('Product.puqProxmox.Update DNS Records')}}
                        </button>

                        <div class="list-group text-start">
                            @foreach($app_instance_info['endpoints'] as $ep)
                                <div class="list-group-item mb-2 shadow-sm rounded">

                                    <div class="d-flex justify-content-between align-items-center">
                                        <h6 class="fw-bold mb-1">
                                            <i class="fas fa-link text-primary me-1"></i> Endpoint
                                        </h6>
                                        @if($ep['show_to_client'])
                                            <span class="badge bg-success">Visible to client</span>
                                        @else
                                    ...
                                    <div class="mt-2 small">
                                        ...
                                        <div class="mt-1">
                                            <strong>Proxy:</strong> {{ $ep['proxy_pass'] }}
                                        </div>
                                    </div>
                    ...
                    @if(!empty($app_instance_info['load_balancer']))

                        @php $lb = $app_instance_info['load_balancer']; @endphp
                        ...
                        <div class="card shadow-sm mb-3 p-2">
                            ...
                            <div class="d-flex align-items-center justify-content-between mb-2 fw-bold">
                                ...
                                <button id="put_web_proxy_config" type="button"
                                        class="btn btn-outline-warning btn-sm d-flex align-items-center">
                                    <i class="fa fa-sync me-1"></i> {{__('Product.puqProxmox.Update WEB Proxy Config')}}
                                </button>
                            </div>

                            <div class="d-flex mb-1">
                                <span class="fw-bold me-2">
                                    <i class="fas fa-server text-info me-1"></i> Name:
                                </span>
                                <span class="text-muted">{{ $lb['name'] }}</span>
                            </div>

                            <div class="d-flex">
                                <span class="fw-bold me-2">
                                    <i class="fas fa-globe text-success me-1"></i> DNS:
                                </span>
                                <a href="https://{{ $lb['dns_record'] }}" target="_blank">
                                    {{ $lb['dns_record'] }}
                                </a>
                            </div>
                    ...
                    @else
                        <p class="text-muted">No load balancer data.</p>
                    @endif


                    @if(!empty($app_instance_info['env_variables']))
                        ...
                        <div class="card shadow-sm mb-3">
                            ...
                            <div id="envCollapse" class="collapse">
                                <div class="card-body">

                                    <div class="list-group text-start">
                                        @foreach($app_instance_info['env_variables'] as $env)
                    ...
                    @else
                        <p class="text-muted">No ENV variables found.</p>
                    @endif
...
{{--<pre>--}}
{{--    {{print_r($lxc_instance->getScriptLogList())}}--}}
{{--</pre>--}}
...
<script>
    $(document).ready(function () {
        ...
        function loadDeployCard() {
            ...
            PUQajax("{{ route('admin.api.Product.puqProxmox.puq_pm_app_instance.deploy_status.get', $app_instance->uuid) }}", {}, 1500, null, 'GET')
                .then(res => {
                    let d = res.data;
                    if (!d) return;
        ...
        $('#put_web_proxy_config').on('click', function () {
            PUQajax('{{route('admin.api.Product.puqProxmox.puq_pm_app_instance.web_proxy_config.put',$app_instance->uuid)}}', {}, 5000, $(this), 'PUT', null);
        });
