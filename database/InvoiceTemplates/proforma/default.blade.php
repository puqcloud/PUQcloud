...
@endphp
    ...
    <!-- HEADER -->
<table style="width: 100%; max-width: 800px; font-family: {{ $pdf_font }}, serif; color: #5d4e37; border-collapse: collapse; background: #fffefc;">
    <tr>
                ...
                <tr>
                    <td style="width: 60%; vertical-align: middle; padding-left: 10px;">
                        ...
                        <div style="font-size: 11px; color: #776655;">
                            <div>Issue Date: <strong>{{ $invoice->issue_date->format('j.m.Y') }}</strong></div>
                            <div>Due Date: <strong>{{ $invoice->due_date->format('j.m.Y') }}</strong></div>
                        </div>
                    </td>
                    <td style="width: 40%; text-align: right; vertical-align: middle; padding-right: 10px;">
                        <div style="background: #fdfaf5; padding: 10px; border: 1px solid #e0d8c5;">
                            <h3 style="margin: 0; font-size: 18px; color: #5d4e37;">Invoice Proforma</h3>
                            <div style="font-size: 13px; color: #5d4e37; font-weight: 600;">#{{ $invoice->number }}</div>
                        </div>
                    </td>
                </tr>
            </table>
...
<br>
<!-- BILL TO / FROM -->
<table style="width: 100%; max-width: 800px; font-family: {{ $pdf_font }}, serif; color: #5d4e37; border-collapse: collapse; background: #fffefc; border: 1px solid #e0d8c5; margin-top: 10px;">
    <thead>
    ...
    <tr>
        <td style="padding: 10px; vertical-align: top; font-size: 12px;">
            <div style="font-weight: 600;">{{ $invoice->client_company_name ?: $invoice->client_firstname . ' ' . $invoice->client_lastname }}</div>
            @if ($invoice->client_address_1) {{ $invoice->client_address_1 }}<br> @endif
            @if ($invoice->client_address_2) {{ $invoice->client_address_2 }}<br> @endif
            @if ($invoice->client_postcode || $invoice->client_city) {{ $invoice->client_postcode }} {{ $invoice->client_city }}<br> @endif
            @if ($invoice->client_country) {{ $invoice->client_country }}<br> @endif
            @if ($invoice->client_tax_id)<small>TAX ID: {{ $invoice->client_tax_id }}</small>@endif
        </td>
        <td style="padding: 10px; vertical-align: top; font-size: 12px;">
            <div style="font-weight: 600;">{{ $invoice->home_company_company_name }}</div>
            @if ($invoice->home_company_address_1) {{ $invoice->home_company_address_1 }}<br> @endif
            @if ($invoice->home_company_address_2) {{ $invoice->home_company_address_2 }}<br> @endif
            @if ($invoice->home_company_postcode || $invoice->home_company_city) {{ $invoice->home_company_postcode }} {{ $invoice->home_company_city }}<br> @endif
            @if ($invoice->home_company_country) {{ $invoice->home_company_country }}<br> @endif
            @if ($invoice->home_company_tax_local_id)<small>TAX ID: {{ $invoice->home_company_tax_local_id }}</small>@endif
        </td>
    </tr>
    </tbody>
</table>
...
<!-- ITEMS TABLE -->
<table style="width: 100%; max-width: 800px; font-family: {{ $pdf_font }}, serif; color: #5d4e37; border-collapse: separate; border-spacing: 0 4px; background: #fffefc; border: 1px solid #e0d8c5;">
    <thead>
    ...
    @foreach ([
        ...
        @if ($tax['amount'] > 0)
            <tr>
                <td style="text-align: right; padding: 4px 12px; font-size: 12px;">{{ $tax['name'] ?: 'Tax' }} ({{ number_format($tax['rate'], 2) }}%)</td>
...
@if ($invoice->transactions && count($invoice->transactions) > 0)
    ...
    <table style="width: 100%; max-width: 800px; font-family: {{ $pdf_font }}, serif; color: #5d4e37; border-collapse: separate; border-spacing: 0 4px; background: #fffefc; margin-top: 10px;">
        <thead>
        <tr style="background: #eae4da; font-size: 13px; text-transform: uppercase;">
