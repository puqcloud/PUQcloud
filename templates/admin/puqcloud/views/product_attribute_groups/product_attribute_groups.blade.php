...
@section('content')
            ...
            <div class="page-title-heading">
                <div>
                    <div class="page-title-head center-elem">
                        ...
                        <span class="d-inline-block">{{__('main.Product Attribute Groups')}}</span>
                    </div>
                        ...
                        <nav class="" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="active breadcrumb-item" aria-current="page">
                                    {{__('main.Product Attribute Groups')}}
                                </li>
                            </ol>
                        </nav>
                    </div>
    ...
    <div class="main-card mb-3 card">
        <div class="card-body">
            <table style="width: 100%;" id="product_attribute_group" class="table table-hover table-striped table-bordered">
                <thead>
                <tr>
                    <th>{{__('main.Icon')}}</th>
                    <th>{{__('main.Key')}}</th>
                    <th>{{__('main.Visible')}}</th>
                    <th>{{__('main.Attributes')}}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            var $tableId = $('#product_attribute_group');
            var ajaxUrl = '{{ route('admin.api.product_attribute_groups.get') }}';
            var columnsConfig = [
                {
                    data: "images",
            ...
            `;
                        ...
                        return `<div class="widget-content p-0">
                                    <div class="widget-content-wrapper">
                                        <div class="widget-content-left">
                                            <div class="widget-heading">${row.key}</div>
                ...
                {name: "attributes_count", data: "attributes_count"},
                {
                    data: 'urls',
                    className: "center",
                    orderable: false,
            ...
            $('#modalSaveButton').on('click', function (event) {
                ...
                if ($('#createForm').length) {
                    ...
                    PUQajax('{{ route('admin.api.product_attribute_group.post') }}', formData, 50, $(this), 'POST', $form)
                        .then(function (response) {
