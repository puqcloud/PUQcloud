...
function renderEditLink(modelUrl) {
    return `<a href="${modelUrl}" class="edit-btn mb-2 me-2 btn-icon-only btn-outline-2x btn btn-outline-warning">
                <i class="fa fa-edit"></i>
            </a>`;
}
...
function initializeTagSelect2(element, {
    url,
    ...
    const defaultOptions = {
        ...
        language: {
            inputTooShort: () => {
                let msg = translate('Enter at least :n characters');
                return msg.replace(':n', minInputLength);
            },
        ...
        ajax: {
            transport: function (params, success, failure) {
                params.data = $.extend(true, {}, params.data, gets);
                PUQajax(url, { term: params.data.term }, timeout, null, 'GET')
                    .then(success)
                    .catch(failure);
            },
            dataType: 'json',
        ...
        templateSelection: function (data) {
            ...
            const color = stringToColor(data.text);
            return $('<span>')
                .text(data.text)
                .css({
    ...
    const mergedOptions = $.extend(true, {}, defaultOptions, select2Options);
    $(element).select2(mergedOptions);
