...
@section('content')
            ...
            <div class="page-title-heading">
                <div>
                    <div class="page-title-head center-elem">
                        ...
                        <span class="d-inline-block">{{__('main.Task Queue')}}</span>
                    </div>
                ...
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="me-3">
                        <select id="intervalSelect" class="form-select">
                            <option value="2">2 {{__('main.seconds')}}</option>
    ...
    <div class="main-card mb-3 card">
        <div class="card-body">
            <table style="width: 100%;" id="taskQueue" class="table table-hover table-striped table-bordered">
                <thead>
                <tr>
                    <th>{{__('main.Job')}}</th>
                    <th>{{__('main.Queue')}}</th>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            var tableId = '#taskQueue';
            var ajaxUrl = '{{ route('admin.api.tasks.get') }}';
            var columnsConfig = [
                {
                    data: 'job_name',
                    ...
                    <div class="widget-content-wrapper">
                        <div class="widget-content-left">
                            <div class="widget-heading">
                                <div class="mb-1 me-1 badge bg-` + getQueueStatusLabelClass(row.status) + `">` + row.status + `</div>`
                            + row.job_name + `</div>
                            <div class="widget-subheading">` + row.job_id + `</div>
                        </div>
                     </div>
                </div>`;
                    }
                ...
                {
                    data: "queue", render: function (data, type, row) {
                        return `<div class="widget-content p-0">
                    <div class="widget-content-wrapper">
                        <div class="widget-content-left">
                            <div class="widget-heading">` + row.queue + `</div>
                            <div class="widget-subheading">` + row.attempts + `/` + row.maxTries + `</div>
                        </div>
                     </div>
                </div>`;
                    }
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.added_at !== null) {
                            result += `<div class="badge ms-0 me-0 badge-dot badge-dot-lg bg-secondary">puq</div>` + formatDateWithoutTimezone(row.added_at) + `<br>`;
                        }
                        if (row.started_at !== null) {
                            result += `<div class="badge ms-0 me-0 badge-dot badge-dot-lg bg-warning">puq</div>` + formatDateWithoutTimezone(row.started_at) + `<br>`;
                        }
                        if (row.completed_at !== null) {
                            result += `<div class="badge ms-0 me-0 badge-dot badge-dot-lg bg-success">puq</div>` + formatDateWithoutTimezone(row.completed_at) + `<br>`;
                        }
                        if (row.filed_at !== null) {
                            result += `<div class="badge ms-0 me-0 badge-dot badge-dot-lg bg-danger">puq</div>` + formatDateWithoutTimezone(row.filed_at);
                        }
                        return result;
                    }
                },
