...
@section('content')
            ...
            <div class="page-title-heading">
                <div>
                    <div class="page-title-head center-elem">
                        ...
                        <span class="d-inline-block">{{__('main.Activity Log')}}</span>
                    </div>
    ...
    <div class="main-card mb-3 card">
        <div class="card-body">
            <table style="width: 100%;" id="activityLogs" class="table table-hover table-striped table-bordered">
                <thead>
                <tr>
                    <th>{{__('main.Date')}}</th>
                    <th>{{__('main.Initializer')}}</th>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            var tableId = '#activityLogs';
            var ajaxUrl = '{{ route('admin.api.activity_logs.get') }}';
            var columnsConfig = [
                {
                        ...
                        <div class="widget-content-left me-3">
                            <div class="avatar-icon-wrapper">
                                ...
                                <div class="avatar-icon rounded">
                                    <img src="${row.initializer.gravatar}" alt="">
                                </div>
                            </div>
                        </div>
                        <div class="widget-content-left">
                            <div class="widget-heading">${row.initializer.firstname} ${row.initializer.lastname}</div>
                            <div class="widget-subheading">${row.initializer.email}</div>
                        </div>
                    </div>
                </div>`;
                        } else {
                ...
                },
                {
                    ...
                    <div class="widget-content-wrapper">
                        <div class="widget-content-left">
                            <div class="widget-heading">${row.action}</div>
                            <div class="badge ${getLogLevelColor(row.level)}">${row.level}</div>
                        </div>
                     </div>
                </div>`;
                    }
        ...
        <strong>${translate('New Data')}:</strong>
        <pre>${data.model_new_data}</pre>
        <strong>${translate('Old Data')}:</strong>
        <pre>${data.model_old_data}</pre>
    `;
