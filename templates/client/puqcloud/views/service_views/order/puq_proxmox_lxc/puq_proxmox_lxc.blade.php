    ...
    <div class="mx-auto" style="max-width: 1600px;">
        <div class="row">
            <div class="col-md-8 mb-2">
                ...
                <div class="card mb-2" id="images" style="min-height: 150px;">
                    <div class="card-header d-flex align-items-center">
                ...
                <div class="main-card mb-2 card">
                    <div class="card-body">
                        <div class="alert alert-info align-items-start gap-2 rounded-4 shadow-sm mb-0" role="alert"
                            ...
                            <div>
                                <p class="mb-1">
                                    {!! __('Product.puqProxmox.Please select <strong>location</strong> and <strong>operating system</strong> first') !!}
                                </p>
                                <p class="mb-0 text-muted">
                                    {{ __('Product.puqProxmox.After that, the list of available servers will appear here') }}
                                </p>
                            </div>
                        </div>
                        <table style="width: 100%; display: none;" id="products"
                            ...
                            <tr>
                                ...
                                <th><i class="fa fa-microchip"></i> {{__('Product.puqProxmox.CPU')}}</th>
                                <th><i class="fa fa-memory"></i> {{__('Product.puqProxmox.RAM')}}</th>
                                <th><i class="fa fa-hdd"></i> {{__('Product.puqProxmox.Disk')}}</th>
                ...
                <div id="productOptions" style="display: none; min-height: 150px;">
                    <form id="orderForm" novalidate="novalidate">
                        ...
                        <input type="hidden" id="rootfs_size_product_option_uuid"
                               name="rootfs_size_product_option_uuid" value="">
                        <input type="hidden" id="mp_size_product_option_uuid"
                               name="mp_size_product_option_uuid" value="">

                        <input type="hidden" id="ipv4_product_option_uuid"
                               name="ipv4_product_option_uuid" value="">
                        <input type="hidden" id="ipv6_product_option_uuid"
                               name="ipv6_product_option_uuid" value="">
                        <input type="hidden" id="local_private_network_product_option_uuid"
                               name="local_private_network_product_option_uuid" value="">
                        <input type="hidden" id="global_private_network_product_option_uuid"
                               name="global_private_network_product_option_uuid" value="">
        ...
        <i class="fa fa-check text-white position-absolute d-none"
           style="top:0; right:0; transform: translate(50%, -50%);
            ...
            function loadImages() {
                ...
                PUQajax(
                    `{{ route('client.api.cloud.group.order.get',['uuid'=>$product_group->uuid,'method'=>'GetImages']) }}`,
                    {},
                    50,
                    null,
                    'GET'
                    ...
                    const imagesData = Object.values(response.data);
                    ...
                    if (imagesData.length > 0) {
                        imagesData.forEach(os => {
                            const versionOptions = os.versions.map(v => {
                                ...
                                return `<option value="${v.value}">${v.name}${priceText}</option>`;
                            }).join('');
                            const card = `
    ...
    <div class="image-widget border rounded p-1 mb-1 position-relative">
        <div class="widget-content p-0 d-flex align-items-center">
            ...
            <div class="widget-content-left flex-grow-1">
                ...
                <select class="form-select form-select-sm mt-1 version-select" data-os="${os.name}">
        ...
        <i class="fa fa-check text-white position-absolute d-none"
           style="top:0; right:0; transform: translate(50%, -50%);
            ...
            function loadProduct() {
                ...
                var columnsConfig = [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row) {
                            let badges = row.cpu_attributes
                                .map(a => `<span class="badge bg-primary px-1 py-1 me-1 mb-1" style="font-size: 0.8rem;">${a}</span>`)
                                .join('');
                            return `<span class="fw-bold" style="font-size: 1.01rem;">${data}</span> <small class="text-muted" style="font-size: 0.9rem;">${data === 1 ? '{{__('Product.puqProxmox.Core')}}' : '{{__('Product.puqProxmox.Cores')}}'}</small> ${badges}`;
                        }
                    },
                    {
                        data: "ram",
                        orderable: false,
                        render: function (data, type, row) {
                            let gb = (data / 1024).toFixed(0);
                            let badges = row.ram_attributes
                                .map(a => `<span class="badge bg-info px-1 py-1 me-1 mb-1" style="font-size: 0.8rem;">${a}</span>`)
                                .join('');
                            return `<span class="fw-bold" style="font-size: 1.01rem;">${gb}</span> <small class="text-muted" style="font-size: 0.9rem;">{{__('Product.puqProxmox.GB')}}</small> ${badges}`;
                        }
                    },
                    {
                        data: "system_disk",
                        ...
                        render: function (data, type, row) {
                            let gb = (data / 1024).toFixed(0);
                            let badges = row.system_disk_attributes
                                .map(a => `<span class="badge bg-secondary px-1 py-1 me-1 mb-1" style="font-size: 0.8rem;">${a}</span>`)
                                .join('');
                            return `<span class="fw-bold" style="font-size: 1.01rem;">${gb}</span> <small class="text-muted" style="font-size: 0.9rem;">{{__('Product.puqProxmox.GB')}}</small> ${badges}`;
                        }
                    },
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <span class="fw-bold text-success" style="font-size: 1.5rem;">${row.price_string}</span>
                            <small class="text-muted" style="font-size: 1rem;">/ {{__('Product.puqProxmox.Monthly')}}</small>
                        </div>
            ...
            function updateValues() {
                ...
                var img = document.getElementById('selectedImage').value;
...
`);
            ...
            function renderImageGroup(group) {
                group.product_options.forEach(opt => {
                    if (opt.value === selectedImage) {
                        $('#os_product_option_uuid').val(opt.uuid);
                        return 0;
                    }
                });
            }
    ...
    `;
        ...
        <div class="col-12 col-sm-6 col-md-6 col-lg-3">
            ...
            function renderRootfsGroup(group) {
                let html = `<div class="card mb-2" id="group_${group.uuid}">`;
                let attributes = group.attributes
                    .map(a => `<span class="badge bg-primary px-1 py-1 me-1 mb-1" style="font-size: 0.8rem;">${a}</span>`)
                    .join('');
    ...
    `;
            ...
            `;
                ...
                <div class="text-nowrap">
                    ...
                    html += `
        <div class="col-12 col-sm-6 col-md-6 col-lg-3">
            <div class="card rootfs-widget p-2 cursor-pointer position-relative"
                 data-uuid="${opt.uuid}" data-group-uuid="${group.uuid}" data-price="${opt.price}"
                    style="cursor:pointer;">
                <div class="d-flex align-items-center">
                    <div class="me-2">
            ...
            function attachRootfsSelection() {
                $('.rootfs-widget').on('click', function () {
                    const optionUuid = $(this).data('uuid');
                    const groupUuid = $(this).data('group-uuid');

                    $(`.rootfs-widget[data-group-uuid="${groupUuid}"] i.fa-check`).addClass('d-none');
                    $(`.rootfs-widget[data-group-uuid="${groupUuid}"]`).removeClass('border-3 border-success fw-bold');

                    $(this).addClass('border-3 border-success fw-bold');
                    $(this).find('i.fa-check').removeClass('d-none');

                    $('#rootfs_size_product_option_uuid').val(optionUuid);

                    updateOrderSummary();
                });

                $('.rootfs-widget').each(function () {
                    const $first = $(this).closest('.row').find('.rootfs-widget').first();

                    $first.addClass('border-3 border-success fw-bold');
                    $first.find('i.fa-check').removeClass('d-none');
                    $('#rootfs_size_product_option_uuid').val($first.data('uuid'));
                });
            }


            function renderAdditionalDiskGroup(group) {
                let html = `<div class="card mb-2" id="group_${group.uuid}">`;
                let attributes = group.attributes
                    .map(a => `<span class="badge bg-primary px-1 py-1 me-1 mb-1" style="font-size: 0.8rem;">${a}</span>`)
                    .join('');
...
style="cursor:pointer;">
            ...
            function attachAdditionalDiskSelection() {
            ...
            function renderNetworkGroupAll(groups) {
                let html = `<div class="card mb-2" id="network_group">
        <div class="card-header">
            <h5 class="mb-0">{{__('Product.puqProxmox.Network')}}</h5>
        </div>
        <div class="card-body">
            <div class="row g-3">`;
                ...
                groups.forEach(group => {
                    ...
                    <span class="text-muted" style="font-size:0.8rem;"> / {{__('Product.puqProxmox.mo')}}</span>
                            ...
                            `<div class="text-nowrap">
                    <span class="disk-price text-success fw-bold">{{__('Product.puqProxmox.Free')}}</span>
                </div>`;
                        ...
                        switch (group.type) {
                            case 'ipv4':
                                ...
                                break;
                            case 'ipv6':
                                iconClass = 'fa fa-network-wired text-primary';
                                break;
                            case 'local_private_network':
                                iconClass = 'fa fa-house';
                                break;
                            case 'global_private_network':
                        ...
                        html += `
            <div class="col-12 col-sm-6 col-md-6 col-lg-3">
                <div class="card network-option p-2 cursor-pointer position-relative border rounded"
                     data-uuid="${opt.uuid}" data-group-type="${group.type}" data-value="${opt.value}">
                    <div class="d-flex align-items-center mb-1">
                        <i class="${iconClass} me-2"></i>
                        <div class="fw-bold">${group.name}</div>
                    </div>
                    ${priceBlock}
                    <input class="form-check-input d-none" type="checkbox" value="${opt.value}" id="opt_${opt.uuid}">
                    <i class="fa fa-check text-white position-absolute d-none"
                       style="top:0; right:0; transform: translate(50%, -50%);
                              background-color: #28a745;
                              border-radius: 50%;
            ...
            function attachNetworkSelectionAll() {
                $('.network-option').on('click', function () {
                    const $checkbox = $(this).find('input[type="checkbox"]');
                    const isChecked = !$checkbox.prop("checked");
                    const type = $(this).data('group-type');
                    const uuid = $(this).data('uuid');

                    $checkbox.prop("checked", isChecked);
                    $(this).toggleClass('border-success border-3', isChecked);
                    $(this).find('i.fa-check').toggleClass('d-none', !isChecked);

                    updateHiddenNetworkFields(type, isChecked ? uuid : '');
                    validateNetworkGroupAll();
                    updateOrderSummary();
                });
            ...
            function updateHiddenNetworkFields(type, value) {
                switch (type) {
                    case 'ipv4':
                        $('#ipv4_product_option_uuid').val(value);
                        break;
                    case 'ipv6':
                        $('#ipv6_product_option_uuid').val(value);
                        break;
                    case 'local_private_network':
                        $('#local_private_network_product_option_uuid').val(value);
                        break;
                    case 'global_private_network':
                        $('#global_private_network_product_option_uuid').val(value);
                        break;
                }
            }

            function validateNetworkGroupAll() {
                const checked = $('.network-option input[type="checkbox"]:checked').length;
                const $groupCard = $('#network_group');

                if (checked === 0) {
                    $groupCard.addClass('border border-danger border-3');
                } else {
                    $groupCard.removeClass('border border-danger border-3');
                }
            }
...
<div class="card mb-2" id="addition_options_block">
    ...
    `;
        ...
        <div class="col-12 col-sm-6 col-md-6 col-lg-3">
                        ...
                        response.data.forEach(group => {
                            switch (group.type) {
                                ...
                                case 'ram':
                                    ...
                                    break;
                                case 'system_disk':
                                    html += renderRootfsGroup(group);
                                    break;
                                case 'additional_disk':
                                    html += renderAdditionalDiskGroup(group);
                                    break;
                                case 'backup_count':
            ...
            function renderSSHKeyBlock(sshKeys = []) {
                return `
    <div class="card mb-2" id="ssh_key_block">
        <div class="card-header">
            <h5 class="mb-0">{{__('Product.puqProxmox.SSH Public Key')}}</h5>
        </div>
        <div class="card-body">
            <label for="puq_pm_ssh_public_key_uuid" class="form-label fw-bold">{{__('Product.puqProxmox.Select Or Custom SSH Key')}}</label>
            <select id="puq_pm_ssh_public_key_uuid" name="puq_pm_ssh_public_key_uuid" class="form-select mb-2">
                <option value="">{{__('Product.puqProxmox.Select SSH Key')}}</option>
                ${sshKeys.map(key => `<option value="${key.uuid}">${key.name}</option>`).join('')}
                <option value="custom">{{__('Product.puqProxmox.Custom Public SSH Key')}}</option>
            </select>
            <textarea id="ssh_public_key_custom" name="ssh_public_key_custom"  class="form-control d-none" rows="4" placeholder="{{__('Product.puqProxmox.Enter Your Public SSH Key')}}"></textarea>
        </div>
    </div>`;
            }

            function attachSSHKeySelection() {
                $('#puq_pm_ssh_public_key_uuid').on('change', function () {
                    const value = $(this).val();
                    $('#ssh_public_key_custom').toggleClass('d-none', value !== 'custom');
                });
            }

            function updateSshKeys() {
                PUQajax(
                    '{{ route('client.api.cloud.group.order.get', ['uuid' => $product_group->uuid, 'method' => 'getSshKeys']) }}', {},
                    100,
                    null,
                    'GET'
                ).then(response => {
                    $('#public_ssh_key').append(renderSSHKeyBlock(response.data));
                    attachSSHKeySelection();
                }).catch(err => {
                    console.error(err);
                });
            }
    ...
    `;
        ...
        `;
            ...
            const requiredFields = [
                ...
                "os_product_option_uuid"
            ];

            const networkFields = [
                "ipv4_product_option_uuid",
                "ipv6_product_option_uuid",
                "local_private_network_product_option_uuid",
            ...
            function checkButtonState() {
                ...
                const oneNetworkFilled = networkFields.some(id => {
                    const val = document.getElementById(id)?.value;
                    return val && val.trim() !== "";
                });

                document.getElementById("btnCompleteProductConfig").disabled = !(allRequiredFilled && oneNetworkFilled);
            }
