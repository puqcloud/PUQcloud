...
<div id="header" class="app-page-title">
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            <a href="{{ route('client.web.panel.cloud.group', $product_group->uuid) }}?page=private_networks"
               class="btn-wide mb-2 me-2 btn btn-outline-2x btn-outline-secondary btn-sm">
                <i class="fa fa-network-wired me-2"></i>
                {{ __('Product.puqProxmox.Manage Private Networks') }}
            </a>

            <a href="{{ route('client.web.panel.cloud.group', $product_group->uuid) }}?page=ssh_public_keys"
               class="btn-wide mb-2 me-2 btn btn-outline-2x btn-outline-secondary btn-sm">
    ...
    <div class="main-card card">
        <div class="card-body">
            <div id="no-servers" class="text-center py-5" style="display: none;">
                ...
                <h3 class="mb-3">{{ __("Product.puqProxmox.You don't have any servers yet") }}</h3>
                <p class="text-muted mb-4">
                    {{ __("Product.puqProxmox.Go ahead and create your first server now – it only takes a few seconds") }}
                </p>
                <a href="{{ route('client.web.panel.cloud.group.order', $product_group->uuid) }}"
        ...
        $(document).ready(function () {
            ...
            function renderBadges(attributes, type) {
                ...
                switch(type) {
                    ...
                    case 'addition_disk': color = 'warning'; break;
                }
            ...
            var columnsConfig = [
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        let domain = row.params.domain || '-';
                        let hostname = row.params.hostname || '-';
                        let ipv4 = row.params.ip_v4 || '-';
                        let ipv6 = row.params.ipv6 || '-';

                        let html = `
            <div class="small fw-semibold">
                <div><strong>{{__('Product.puqProxmox.Domain')}}:</strong> ${domain}</div>
                <div><strong>{{__('Product.puqProxmox.IPv4')}}:</strong> ${ipv4}</div>
                <div><strong>{{__('Product.puqProxmox.IPv6')}}:</strong> ${ipv6}</div>
            </div>
        `;
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        let main = row.params.system_disk;
                        let add = row.params.addition_disk;
                        ...
                        <small class="text-muted" style="white-space: nowrap;">{{__('Product.puqProxmox.GB')}} {{__('Product.puqProxmox.Main')}}</small>
                    </div>
                    <div class="mt-1" style="white-space: nowrap;">${renderBadges(row.params.system_disk_attributes, 'system_disk')}</div>`;

                        if (add && add !== "" && Number(add) > 0) {
                            let addGb = (add / 1024).toFixed(0);
                            html += `<div style="white-space: nowrap;">
                        <span class="fw-bold" style="white-space: nowrap;">${addGb}</span>
                        <small class="text-muted" style="white-space: nowrap;">{{__('Product.puqProxmox.GB')}} {{__('Product.puqProxmox.Additional')}}</small>
                    </div>
                    <div class="mt-1" style="white-space: nowrap;">${renderBadges(row.params.addition_disk_attributes, 'addition_disk')}</div>`;
                        }
