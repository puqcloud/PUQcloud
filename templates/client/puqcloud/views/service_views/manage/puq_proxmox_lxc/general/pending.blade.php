        ...
        <div class="card-body text-center" id="status">
            @if($service->create_error === 'Insufficient funds')
                <div class="d-flex justify-content-center align-items-center" style="height: 300px;">
                    <div class="alert alert-danger text-uppercase fw-bold d-flex align-items-center justify-content-center p-4 shadow-lg"
                         role="alert" style="font-size:1.3rem; border: 2px solid #dc3545; animation: pulse 1.5s infinite;">
                        <i class="fa-solid fa-triangle-exclamation me-2" style="font-size:1.5rem;"></i>
                        {{ __('main.Insufficient funds') }}
                    </div>
                </div>
            @else
                <div class="mb-4">
                ...
                <h4 class="fw-bold text-dark" id="status_message">
                    {{ __('Product.puqProxmox.AutomaticDeployment') }}<br>
                    {{ __('Product.puqProxmox.PleaseWait') }}...
                </h4>

                <p class="small text-muted mt-3" id="sub_message">
                    {{ __('Product.puqProxmox.PreparingYourService') }}
                </p>
                ...
                <p class="small text-muted mt-2" id="timer_text">
                    {{ __('Product.puqProxmox.CheckingAgainIn') }} <span id="countdown">5</span> {{ __('Product.puqProxmox.Seconds') }}
                </p>
            @endif
        </div>
    </div>
    ...
    @if($service->create_error !== 'Insufficient funds')
        <script>
            $(document).ready(function () {
                ...
                let messages = [
                    "{{ __('Product.puqProxmox.PreparingYourService') }}",
                    "{{ __('Product.puqProxmox.AllocatingResources') }}",
                    "{{ __('Product.puqProxmox.ConfiguringSettings') }}",
                    "{{ __('Product.puqProxmox.FinalizingDeployment') }}"
                ];
                let msgIndex = 0;
                ...
                function checkStatus() {
                    PUQajax("{{ route('client.api.cloud.service.manage.get', ['uuid' => $service->uuid, 'method' => 'GetStatus']) }}", {}, 50, null, 'GET')
