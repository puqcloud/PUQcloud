...
class PanelController extends Controller
{
    ...
    public function moduleClientWeb(Request $request, $type, $name, $method, $uuid = null): View|Factory|Application
    {
        ...
        if ($type == 'Payment') {
            ...
            $module_data = $payment_gateway->module->moduleExecute('controllerClientWeb_'.$method, ['request' => $request]);
    ...
    public function moduleClientStatic(Request $request, $type, $name, $method, $uuid = null): JsonResponse
    {
        ...
        if ($type == 'Payment') {
            ...
            $result = $payment_gateway->module->moduleExecute('controllerClientStatic_'.$method, ['request' => $request]);
            if ($result['status'] === 'success') {
                return $result['data'];
            }
    ...
    public function dashboardServicesApi(): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'termination_request' => $client->services()->whereIn('status', ['active', 'suspended', 'pending'])->where('termination_request', true)->count(),
            ],
        ]);
    }
